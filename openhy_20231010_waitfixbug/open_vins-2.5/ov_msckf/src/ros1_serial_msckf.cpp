...
int main(int argc, char **argv) {
  ...
  if (params.use_stereo && params.state_options.num_cameras == 2) {
    ...
    PRINT_DEBUG("serial cam (stereo): %s\n", cam_topic1.c_str());
  } else if(params.use_stereo && params.state_options.num_cameras == 3){
    // Read in the topics
    std::string cam_topic0, cam_topic1, cam_topic2;
    nh->param<std::string>("topic_camera" + std::to_string(0), cam_topic0, "/cam" + std::to_string(0) + "/image_raw");
    nh->param<std::string>("topic_camera" + std::to_string(1), cam_topic1, "/cam" + std::to_string(1) + "/image_raw");
    nh->param<std::string>("topic_camera" + std::to_string(2), cam_topic2, "/cam" + std::to_string(2) + "/image_raw");
    parser->parse_external("relative_config_imucam", "cam" + std::to_string(0), "rostopic", cam_topic0);
    parser->parse_external("relative_config_imucam", "cam" + std::to_string(1), "rostopic", cam_topic1);
    parser->parse_external("relative_config_imucam", "cam" + std::to_string(2), "rostopic", cam_topic2);
    topic_cameras.emplace_back(0, cam_topic0);
    topic_cameras.emplace_back(1, cam_topic1);
    topic_cameras.emplace_back(2, cam_topic2);
    PRINT_DEBUG("serial cam (stereo): %s\n", cam_topic0.c_str());
    PRINT_DEBUG("serial cam (stereo): %s\n", cam_topic1.c_str());
    PRINT_DEBUG("serial cam (stereo): %s\n", cam_topic2.c_str());
  }else {
  ...
  //===================================================================================
  //===================================================================================
  std::thread thsys51(&Optimizer::globalBA,sys->opt);
  std::thread thsys52(&Optimizer::globalBA1,sys->opt);
  ...
  while (ros::ok()) {
    ...
    if (params.state_options.num_cameras == 2) {
    ...
    } else if (params.state_options.num_cameras == 3 && params.use_stereo) {

      // Now lets do some logic to find two images which are next to each other
      // We want to ensure that our stereo pair are very close to occurring at the same time
      double half_time_delta = 0.5*1.0 / 20;        //params.track_frequency;
      bool have_found_pair = false;//双目配准
      bool have_sametime0to2 = false;//0,2对齐
      while ((!have_found_pair ||!have_sametime0to2)&&view_cameras_iterators.at(0) != view_cameras.at(0)->end() &&
             view_cameras_iterators.at(1) != view_cameras.at(1)->end()&& view_cameras_iterators.at(2) != view_cameras.at(2)->end()) {

           // Get left and right cameras
         auto msg_cam0 = msg_images_current.at(0);
         auto msg_cam1 = msg_images_current.at(1);
         auto msg_cam2 = msg_images_current.at(2);
         auto msg_cam0_next = msg_images_next.at(0);
         auto msg_cam1_next = msg_images_next.at(1);
         ...
         double time0 = msg_cam0->header.stamp.toSec();
         double time1 = msg_cam1->header.stamp.toSec();
         double time2 = msg_cam2->header.stamp.toSec();
         double time0_next = msg_cam0_next->header.stamp.toSec();
         double time1_next = msg_cam1_next->header.stamp.toSec();
        ...
        //第三目的时间对齐
        if(abs(time2-time0)<=half_time_delta){
          have_sametime0to2 = true;
        }else if (time2> time0)  {
          have_found_pair = false;//0又发生了变动 
           msg_images_current.at(0) = msg_images_next.at(0);
          view_cameras_iterators.at(0)++;
          if(view_cameras_iterators.at(0) != view_cameras.at(0)->end()) {
            msg_images_next.at(0) = view_cameras_iterators.at(0)->instantiate<sensor_msgs::Image>();
          ...
          msg_images_current.at(2) = msg_images_next.at(2);
          view_cameras_iterators.at(2)++;
          if(view_cameras_iterators.at(2) != view_cameras.at(2)->end()) {
            msg_images_next.at(2) = view_cameras_iterators.at(2)->instantiate<sensor_msgs::Image>();
          }
        }
     }//while0to2
      ...
      // Break out if we have ended
      if (view_cameras_iterators.at(0) == view_cameras.at(0)->end() || view_cameras_iterators.at(1) == view_cameras.at(1)->end() || 
        view_cameras_iterators.at(2) == view_cameras.at(2)->end()) {
        break;
      }
      ...
      // Feed it into our system
      viz->callback_threecameras(msg_images_current.at(0), msg_images_current.at(1), msg_images_current.at(2), 0, 1, 2);
