...
VioManager::VioManager(VioManagerOptions &params_) {
  ...
  if (params.use_klt) {
    trackFEATS = std::shared_ptr<TrackBase>(new TrackKLT(state->_cam_intrinsics_cameras, params.init_options.init_max_features,
                                                         ...
                                                         params.min_px_dist_mid,params.fast_threshold_max,params.fast_threshold_min));
  } else {
  ...
  // Optimizer for end
  // opt = std::make_shared<Optimizer>(params);
  opt = std::make_shared<Optimizer>(params.state_options.max_clone_size);
  opt->max_cameras = params.state_options.num_cameras;
  std::vector<double> imuNoises{params.imu_noises.sigma_a, params.imu_noises.sigma_ab, params.imu_noises.sigma_w, params.imu_noises.sigma_wb};
  //cam intrinsics
  std::unordered_map<size_t, std::shared_ptr<ov_core::CamBase>> camaraIntris = params.camera_intrinsics;
  opt->initOptimizerParameters(imuNoises, camaraIntris, params.imu_noises.freq_imu, params.gravity_mag);
}
...
void VioManager::feed_measurement_imu(const ov_core::ImuData &message) {
  ...
  // Push back to our optimizer
  opt->feed_imu(message);
...
void VioManager::track_image_and_update(const ov_core::CameraData &message_const) {
  ...
  if (is_initialized_vio) {
    ...
    trackDATABASE->append_new_measurements(trackFEATS->get_mid_feature_database());
    // opt->get_feature_database()->append_new_measurements(trackFEATS->get_feature_database());
  }
  ...
  if (is_initialized_vio && updaterZUPT != nullptr && (!params.zupt_only_at_beginning || !has_moved_since_zupt)) {
    ...
    if (did_zupt_update) {
      if (message.sensor_ids.at(0) == 0) {
        ...
        retriangulate_active_tracks(message,true);
      }
...
void VioManager::do_feature_propagate_update(const ov_core::CameraData &message) {
  ...
  // opt->get_feature_database()->append_new_tri(featsup_MSCKF);
  ...
  // opt->get_feature_database()->append_new_tri(feats_slam_DELAYED);
  ...
  // Finally send optimizer msg to end
  ...
  std::stringstream ss;
  ss << "[TIME]: " << std::setprecision(4) << time_total  << "("<<state->_timestamp << ")" << " seconds for total (camera";
  for (const auto &id : message.sensor_ids) {
    ss << " " << id;
  }
  ss << ")" << std::endl;
...
void VioManager::retriangulate_active_tracks(const ov_core::CameraData &message,bool only_clean) {
  ...
  // opt->get_feature_database()->append_new_tri(active_features);
