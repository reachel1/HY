...
struct VioManagerOptions {
  ...
  void print_and_load_noise(const std::shared_ptr<ov_core::YamlParser> &parser = nullptr) {
    ...
    if (parser != nullptr) {
      ...
      parser->parse_external("relative_config_imu", "imu0", "update_rate", imu_noises.freq_imu);
  ...
  int fast_threshold_max = 30;
  int fast_threshold_min = 15;
  ...
  void print_and_load_trackers(const std::shared_ptr<ov_core::YamlParser> &parser = nullptr) {
    if (parser != nullptr) {
      ...
      parser->parse_config("fast_threshold_max", fast_threshold_max);
      parser->parse_config("fast_threshold_min", fast_threshold_min);
      parser->parse_config("grid_x", grid_x);
      parser->parse_config("grid_y", grid_y);
      parser->parse_config("min_px_dist", min_px_dist);
      parser->parse_config("min_px_dist_mid", min_px_dist_mid);
    ...
    PRINT_DEBUG("  - fast threshold: %d(%d,%d)\n", fast_threshold,fast_threshold_max,fast_threshold_min);
    PRINT_DEBUG("  - grid X by Y: %d by %d\n", grid_x, grid_y);
    PRINT_DEBUG("  - min px dist(min px dist mid): %d(%d)\n", min_px_dist,min_px_dist_mid);
