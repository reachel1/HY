...
ROS1Visualizer::ROS1Visualizer(std::shared_ptr<ros::NodeHandle> nh, std::shared_ptr<VioManager> app, std::shared_ptr<Simulator> sim)
  ...
  pub_posegps = nh->advertise<geometry_msgs::PoseStamped>("/ov_msckf/posegps", 2);
  PRINT_DEBUG("Publishing: %s\n", pub_posegps.getTopic().c_str());
...
void ROS1Visualizer::set_calib(std::string stereo_calib){
  // camera distort
  /// set stereo calib
  cv::FileStorage fsSettings(stereo_calib, cv::FileStorage::READ);
  if(!fsSettings.isOpened())
  {
    PRINT_ERROR(RED "ERROR: Wrong path to settings" RESET);
    std::exit(EXIT_FAILURE);
  }
  cv::Mat K_l, K_r, P_l, P_r, R_l, R_r, D_l, D_r, K_m, P_m, R_m, D_m;
  ...
  fsSettings["MID.K"] >> K_m;
  fsSettings["MID.P"] >> P_m;
  fsSettings["MID.R"] >> R_m;
  fsSettings["MID.D"] >> D_m;
  int rows_l = fsSettings["LEFT.height"];
  int cols_l = fsSettings["LEFT.width"];
  int rows_r = fsSettings["RIGHT.height"];
  int cols_r = fsSettings["RIGHT.width"];
  int rows_m = fsSettings["MID.height"];
  int cols_m = fsSettings["MID.width"];
  if(K_l.empty() || K_r.empty() || P_l.empty() || P_r.empty() || 
              R_l.empty() || R_r.empty() || D_l.empty() || D_r.empty() ||
              K_m.empty() || P_m.empty() || R_m.empty() || D_m.empty() ||
              rows_l==0 || rows_r==0 || cols_l==0 || cols_r==0 || rows_m==0 || cols_m==0)
  {
    PRINT_INFO(RED "ERROR: Calibration parameters to rectify stereo are missing!"  RESET);
    std::exit(EXIT_FAILURE);
  }
  cv::initUndistortRectifyMap(K_l,D_l,R_l,P_l.rowRange(0,3).colRange(0,3),cv::Size(cols_l,rows_l),CV_32F,M1l,M2l);
  cv::initUndistortRectifyMap(K_r,D_r,R_r,P_r.rowRange(0,3).colRange(0,3),cv::Size(cols_r,rows_r),CV_32F,M1r,M2r);
  cv::initUndistortRectifyMap(K_m,D_m,R_m,P_m.rowRange(0,3).colRange(0,3),cv::Size(cols_r,rows_r),CV_32F,M1m,M2m); 
}
...
void ROS1Visualizer::setup_subscribers(std::shared_ptr<ov_core::YamlParser> parser) {
  ...
  _nh->param<std::string>("topic_rtk", topic_rtk, "/rtk");
  _nh->param<std::string>("topic_gps", topic_gps, "/gps");
  parser->parse_external("relative_config_imu", "imu0", "rostopic", topic_imu);
  parser->parse_external("relative_config_imu", "rtk0", "rostopic", topic_rtk);
  parser->parse_external("relative_config_imu", "gps0", "rostopic", topic_gps);
  sub_imu = _nh->subscribe(topic_imu, 300, &ROS1Visualizer::callback_inertial,this);
  sub_rtk = _nh->subscribe(topic_rtk, 200, &ROS1Visualizer::callback_rtk,this);
  sub_gps = _nh->subscribe(topic_gps, 100, &ROS1Visualizer::callback_gps,this);
  PRINT_INFO("subscribing to imu: %s\n", topic_imu.c_str());
  PRINT_INFO("subscribing to rtk: %s\n", topic_rtk.c_str());
  ...
  if (_app->get_params().state_options.num_cameras == 2) {
    ...
    auto sync3 = std::make_shared<message_filters::Synchronizer<sync_pol3>>(sync_pol3(10), *image_sub0, *image_sub1,*image_sub2);
    sync3->registerCallback(boost::bind(&ROS1Visualizer::callback_threecameras, this, _1, _2, _3, 0, 1, 2));
    // Append to our vector of subscribers
    sync3_cam.push_back(sync3);
    sync_subs_cam.push_back(image_sub0);
    sync_subs_cam.push_back(image_sub1);
...
void ROS1Visualizer::visualize_final() {
  ...
  PRINT_INFO(REDPURPLE "[VIO]: feed %d pic,track %d cam\n\n" RESET,pic_num,_app->cam_num);
...
void ROS1Visualizer::callback_threecameras(const sensor_msgs::ImageConstPtr &msg0, const sensor_msgs::ImageConstPtr &msg1, const sensor_msgs::ImageConstPtr &msg2, int cam_id0,
  ...
  cv::Mat im0,im1,im2;
  cv::remap(cv_ptr0->image.clone(),im0,M1l,M2l,cv::INTER_LINEAR);
  cv::remap(cv_ptr1->image.clone(),im1,M1r,M2r,cv::INTER_LINEAR);
  cv::remap(cv_ptr2->image.clone(),im2,M1m,M2m,cv::INTER_LINEAR);
  message.images.push_back(im0);
  message.images.push_back(im1);
  message.images.push_back(im2);

  // Load the mask if we are using it, else it is empty
  // TODO: in the future we should get this from external pixel segmentation
  // cv::Mat mask_mid = cv::imread(ros::package::getPath("ov_msckf") + "/mask_mid.jpg");
  // message.masks.push_back(cv::Mat(cv_ptr0->image.rows, cv_ptr0->image.cols, CV_8UC1, cv::Scalar(255)));
  message.masks.push_back(cv::Mat::zeros(cv_ptr0->image.clone().rows, cv_ptr0->image.clone().cols, CV_8UC1));
  message.masks.push_back(cv::Mat::zeros(cv_ptr1->image.clone().rows, cv_ptr1->image.clone().cols, CV_8UC1));
  message.masks.push_back(cv::Mat::zeros(cv_ptr2->image.clone().rows, cv_ptr2->image.clone().cols, CV_8UC1));
...
void ROS1Visualizer::callback_rtk(const novatel_oem7_msgs::INSPVAX::ConstPtr &msg){
  //check imu init
  if (!_app->initialized() || msg->header.stamp.toSec() < _app->get_startup_time())
    return;
  //check rtk init and change rtk to T
  Eigen::Matrix<double,6,1> data;
  data<<msg->latitude,msg->longitude,msg->height,msg->roll/180*PI,msg->pitch/180*PI,-msg->azimuth/180*PI;
  double local_E,local_N,local_U;
  Eigen::Matrix4d T;
  if(!rtk_init){
    geo_converter.Reset(data(0), data(1), data(2));
    rtk_init = true;
    T_init = Eigen::Matrix4d::Identity();
    T_BI = Eigen::Matrix4d::Identity();
    T_BI(0,0) = -1;
    T_BI(2,2) = -1;
    geo_converter.Forward(data(0), data(1), data(2), local_E, local_N, local_U);
    Eigen::Vector3d t(local_E, local_N, local_U);
    T_init.block(0,0,3,3) = ov_core::rot_z(data(5)) * ov_core::rot_y(data(4)) * ov_core::rot_x(data(3));
    T_init.block(0,3,3,1) = t;
    T = TI_init;
  }
  else{
    geo_converter.Forward(data(0), data(1), data(2), local_E, local_N, local_U);
    ...
    T_now.block(0,0,3,3) = ov_core::rot_z(data(5)) * ov_core::rot_y(data(4)) * ov_core::rot_x(data(3));
    T_now.block(0,3,3,1) = t;
    T = (TI_init * (T_BI * (ov_core::Inv_se3(T_init) * T_now))) * ov_core::Inv_se3(T_BI);
    //T = TI_init * ov_core::Inv_se3(T_init) * T_now;
  }
  Eigen::Matrix<double,8,1> state_gt;
  state_gt(0,0) = msg->header.stamp.toSec();
  state_gt.block(1,0,4,1) = ov_core::rot_2_quat(T.block(0,0,3,3).transpose());
  state_gt.block(5,0,3,1) = T.block(0,3,3,1);
  // Create pose of gt
  geometry_msgs::PoseStamped poseIinM;
  poseIinM.header.stamp = msg->header.stamp;
...
void ROS1Visualizer::callback_gps(const novatel_oem7_msgs::BESTGNSSPOS::ConstPtr &msg){
  //check imu init
  if (!_app->initialized() || !rtk_init)
    return;
  //check rtk init and change rtk to T
  Eigen::Matrix<double,3,1> data;
  data<<msg->lat,msg->lon,msg->hgt;
  double local_E,local_N,local_U;
  Eigen::Matrix4d T_now = Eigen::Matrix4d::Identity();
  geo_converter.Forward(data(0), data(1), data(2), local_E, local_N, local_U);
  Eigen::Vector3d t(local_E, local_N, local_U);
  T_now.block(0,3,3,1) = t;
  Eigen::Matrix4d T = (TI_init * (T_BI * (ov_core::Inv_se3(T_init) * T_now))) * ov_core::Inv_se3(T_BI);
  Eigen::Matrix<double,8,1> state_gt;
  state_gt(0,0) = msg->header.stamp.toSec();
  state_gt.block(1,0,4,1) = ov_core::rot_2_quat(Eigen::Matrix3d::Identity());
  state_gt.block(5,0,3,1) = T.block(0,3,3,1);
  // Create pose of gt
  geometry_msgs::PoseStamped poseIinM;
  poseIinM.header.stamp = msg->header.stamp;
...
void ROS1Visualizer::publish_state() {
  ...
  if(!imu_init){
    imu_init = true;
    TI_init = Eigen::Matrix4d::Identity();
    TI_init.block(0,0,3,3) = state->_imu->Rot().transpose();
    TI_init.block(0,3,3,1) = state->_imu->pos();
  }
  ...
  // Finally set the covariance in the message (in the order position then orientation as per ros convention)
  poseIinM.pose.covariance[0] = state->_imu->vel()(0);
  poseIinM.pose.covariance[1] = state->_imu->vel()(1);
  poseIinM.pose.covariance[2] = state->_imu->vel()(2);
  poseIinM.pose.covariance[3] = state->_imu->bias_a()(0);
  poseIinM.pose.covariance[4] = state->_imu->bias_a()(1);
  poseIinM.pose.covariance[5] = state->_imu->bias_a()(2);
  poseIinM.pose.covariance[6] = state->_imu->bias_g()(0);
  poseIinM.pose.covariance[7] = state->_imu->bias_g()(1);
  poseIinM.pose.covariance[8] = state->_imu->bias_g()(2);
