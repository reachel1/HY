...
#include "../../thirdparty/geographiclib/include/GeographicLib/LocalCartesian.hpp"
...
public:
  ...
  /// Callback for synchronized stereo three camera information
  void callback_threecameras(const sensor_msgs::ImageConstPtr &msg0, const sensor_msgs::ImageConstPtr &msg1, const sensor_msgs::ImageConstPtr &msg2, int cam_id0,
                                     int cam_id1,int cam_id2);

  /// Callback for rtk msg
  void callback_rtk(const novatel_oem7_msgs::INSPVAX::ConstPtr &msg);

  /// Callback for gps msg
  void callback_gps(const novatel_oem7_msgs::BESTGNSSPOS::ConstPtr &msg);

  /// set calib for distort three cameras
  void set_calib(std::string stereo_calib);
...
protected:
  ...
  ros::Subscriber sub_imu,sub_rtk,sub_gps;
  ...
  std::vector<std::shared_ptr<message_filters::Synchronizer<sync_pol3>>> sync3_cam;
  std::vector<std::shared_ptr<message_filters::Subscriber<sensor_msgs::Image>>> sync_subs_cam;

  
  /// stereo rect matrix
  cv::Mat M1l,M2l,M1r,M2r,M1m,M2m;
  ...
  ros::Publisher pub_pathgt, pub_posegt, pub_posegps;
  ...
  bool rtk_init = false,imu_init = false;
  Eigen::Matrix4d T_init,TI_init,T_BI;
