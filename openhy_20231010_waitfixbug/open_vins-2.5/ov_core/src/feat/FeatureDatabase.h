...
public:
  ...
  void cleanup_measurements(double timestamp) {
    std::unique_lock<std::mutex> lck(mtx);
    std::vector<std::shared_ptr<Feature>> feat_del;
    for (auto it = features_idlookup.begin(); it != features_idlookup.end();) {
  ...
  void append_new_tri(std::vector<std::shared_ptr<Feature>> &active_feats){
    std::unique_lock<std::mutex> lck(mtx);
    if (active_feats.empty())
      return;
    auto it = active_feats.begin();
    while(it != active_feats.end()){
      if (features_idlookup.find((*it)->featid) != features_idlookup.end()){
        std::shared_ptr<Feature> temp = features_idlookup.at((*it)->featid);
        temp->p_FinA = (*it)->p_FinA;
        temp->p_FinG = (*it)->p_FinG;
        temp->triangule = true;
      }
      it++;
    }
