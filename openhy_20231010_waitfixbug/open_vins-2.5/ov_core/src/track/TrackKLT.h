...
public:
  ...
  explicit TrackKLT(std::unordered_map<size_t, std::shared_ptr<CamBase>> cameras, int numfeats, int numaruco, bool binocular,
                    ...
                    int minpxdistmid = 60,int thresholdmax = 30,int thresholdmin = 15)
      : TrackBase(cameras, numfeats, numaruco, binocular, histmethod), threshold(fast_threshold), grid_x(gridx), grid_y(gridy),
        min_px_dist(minpxdist), min_px_dist_mid(minpxdistmid), threshold_max(thresholdmax), threshold_min(thresholdmin){}
...
protected:
  ...
  /// @brief Process mid mono image and stereo pair of images
  /// @param message Contains our timestamp, images, and camera ids
  /// @param msg_id_left first image index in message data vector
  /// @param msg_id_right second image index in message data vector
  /// @param msg_id_mid third image index in message data vector
  void feed_threestereo(const CameraData &message, size_t msg_id_left, size_t msg_id_right,size_t msg_id_mid);
  ...
  /// @brief detection for enough feat to track
  /// @param imgpyr0 pyramid for detection
  /// @param cam_id camera sensor id
  /// @param pts0 vector of  extracted keypoints
  /// @param ids0 feature ids for each currently extracted keypoint
  void perform_detection(const std::vector<cv::Mat> &imgpyr0,
  ...
  /// @brief KLT track between four images(two pair of stereo images)
  /// @param img0pyr current frame left img pyramid
  /// @param img1pyr current frame right img pyramid
  /// @param img0pyr_last last frame left img pyramid
  /// @param img1pyr_last last frame right img pyramid
  /// @param pts0 points we tracked in left
  /// @param pts1 points we tracked in right
  /// @param pts0_last points we have tracked in last and add in left
  /// @param pts1_last points we have tracked in last and add(nothing)
  /// @param id0 id of the left camera
  /// @param id1 id of the right camera
  /// @param mask_out what points had valid tracks
  void perform_matching_stereo(const std::vector<cv::Mat> &img0pyr, const std::vector<cv::Mat> &img1pyr, 
