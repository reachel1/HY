...
void TrackBase::display_history(cv::Mat &img_out, int r1, int g1, int b1, int r2, int g2, int b2, std::vector<size_t> highlighted) {
  ...
  // If new, then resize the current image
  if (image_new)
    img_out = cv::Mat(max_height*3, (int)img_last_cache.size() * max_width, CV_8UC3, cv::Scalar(0, 0, 0));
  ...
  for (auto const &pair : img_last_cache) {
    ...
    std::unique_lock<std::mutex> lck(mtx_feeds.at(pair.first));
    // select the subset of the image
    cv::Mat img_temp,img_temp2,img_temp3;
    if (image_new){
      cv::cvtColor(img_last_cache[pair.first], img_temp, cv::COLOR_GRAY2RGB);
      cv::cvtColor(img_last_cache[pair.first], img_temp2, cv::COLOR_GRAY2RGB);
      cv::cvtColor(img_last_cache[pair.first], img_temp3, cv::COLOR_GRAY2RGB);
    }
    else{
      img_temp = img_out(cv::Rect(max_width * index_cam, 0, max_width, max_height));
      img_temp2 = img_out(cv::Rect(max_width * index_cam, 0, max_width, max_height));
      img_temp3 = img_out(cv::Rect(max_width * index_cam, 0, max_width, max_height));
    }
    // draw, loop through all keypoints
    for (size_t i = 0; i < ids_last[pair.first].size(); i++) {
      ...
      size_t z = feat->uvs[pair.first].size() - 1;
      ...
      cv::circle(img_temp, pt_c, (is_small) ? 2 : 4, cv::Scalar(color_r, color_g, color_b), cv::FILLED);
      cv::circle(img_temp2, pt_c, (is_small) ? 2 : 4, cv::Scalar((feat->featid/2)%65025%255, feat->featid%65025/255, feat->featid%65025%255), cv::FILLED);
      if(is_stereo){
        cv::Point2f pt_l_top = cv::Point2f(pt_c.x - ((is_small) ? 3 : 5), pt_c.y - ((is_small) ? 3 : 5));
        cv::Point2f pt_l_bot = cv::Point2f(pt_c.x + ((is_small) ? 3 : 5), pt_c.y + ((is_small) ? 3 : 5));
        cv::rectangle(img_temp2, pt_l_top, pt_l_bot, cv::Scalar(0, 255, 0), 1);
      }
    }
    // Draw what camera this is
    auto txtpt = (is_small) ? cv::Point(10, 30) : cv::Point(30, 60);
    ...
    // Replace the output image
    img_temp.copyTo(img_out(cv::Rect(max_width * index_cam, 0, img_last_cache[pair.first].cols, img_last_cache[pair.first].rows)));
    img_temp2.copyTo(img_out(cv::Rect(max_width * index_cam, img_last_cache[pair.first].rows, img_last_cache[pair.first].cols, img_last_cache[pair.first].rows)));
    for(int m = 0;m<img_last_cache[pair.first].rows;m+=15){
      cv::Point2f pt_l_left = cv::Point2f(0, m);
      cv::Point2f pt_l_right = cv::Point2f(img_last_cache[pair.first].cols,m);
      cv::line(img_temp3, pt_l_left, pt_l_right, cv::Scalar(255, 0, 0));
    }
    img_temp3.copyTo(img_out(cv::Rect(max_width * index_cam, img_last_cache[pair.first].rows * 2, img_last_cache[pair.first].cols, img_last_cache[pair.first].rows)));
    index_cam++;
  }
}
