...
public:
  ...
  TrackBase(std::unordered_map<size_t, std::shared_ptr<CamBase>> cameras, int numfeats, int numaruco, bool stereo,
      ...
      : camera_calib(cameras), database(new FeatureDatabase()), mid_database(new FeatureDatabase())
  ...
  std::shared_ptr<FeatureDatabase> get_mid_feature_database() { return mid_database;}
  /**
...
protected:
  ...
  /// Database with mono mid features
  std::shared_ptr<FeatureDatabase> mid_database;
