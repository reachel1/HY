...
public:
  ...
  /// @brief function for FAST detection
  /// @param img img to detect Fast feature
  /// @param pts input and output feature
  /// @param grid_2d_0 feature occurpyid on the img 
  /// @param min_px_dist the min px dist to to be "far away enough" to be a different extracted feature
  /// @param threshold Fast threshold
  /// @param nonmaxSuppression is or not do nonmaxSuppression
  static void FAST_detection(const cv::Mat &img, std::vector<cv::KeyPoint> &pts, int feat_num,
        int min_px_dist, int threshold_max, int threshold_min, bool nonmaxSuppression) {
    
    // Calculate the size our extraction boxes should be
    int size_x = min_px_dist;
    int size_y = min_px_dist;
    ...
    // Parallelize our 2d grid extraction!!
    int ct_cols = std::floor(img.cols / min_px_dist);
    int ct_rows = std::floor(img.rows / min_px_dist);
    int num_features_grid = feat_num / ct_cols / ct_rows + 1;
    std::vector<std::vector<cv::KeyPoint>> collection(ct_cols * ct_rows);
    parallel_for_(cv::Range(0, ct_cols * ct_rows), LambdaBody([&](const cv::Range &range) {
                    for (int r = range.start; r < range.end; r++) {
                      // Calculate what cell xy value we are in
                      ...
                      // Extract FAST features for this part of the image
                      std::vector<cv::KeyPoint> pts_new;
                      while(pts_new.size() == 0 && th_fast >= threshold_min){
                        cv::FAST(img(img_roi), pts_new, th_fast, nonmaxSuppression);
                        th_fast = th_fast - 5;
                      }
                      // Now lets get the top number from this
                      std::sort(pts_new.begin(), pts_new.end(), Grider_FAST::compare_response);
