...
struct Pose_and_calib {

  /// Timestamp of the reading
  double timestamp;

  /// Camera ids for each of the images collected
  Eigen::Matrix3d R,R_calib0,R_calib1,R_calib2;
  Eigen::Vector3d t,t_calib0,t_calib1,t_calib2;

  int vio;

  Eigen::Vector3d gps_data, velocity, bias_a, bias_g;

  bool operator<(const Pose_and_calib &other) const { 
    return timestamp < other.timestamp;
  }
