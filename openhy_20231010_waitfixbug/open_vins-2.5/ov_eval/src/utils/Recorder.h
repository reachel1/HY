...
public:
  ...
  void callback_posecovariance(const geometry_msgs::PoseWithCovarianceStampedPtr &msg) {
    ...
    q_ItoG << msg->pose.pose.orientation.x, msg->pose.pose.orientation.y, msg->pose.pose.orientation.z, msg->pose.pose.orientation.w;
    p_IinG << msg->pose.pose.position.x, msg->pose.pose.position.y, msg->pose.pose.position.z;
    v_ba_bg << msg->pose.covariance.at(0), msg->pose.covariance.at(1), msg->pose.covariance.at(2), msg->pose.covariance.at(3),
        msg->pose.covariance.at(4), msg->pose.covariance.at(5), msg->pose.covariance.at(6), msg->pose.covariance.at(7),
        msg->pose.covariance.at(8);
    has_covariance = true;
    write();
  }
...
protected:
  ...
  void write() {
    ...
    if (has_covariance) {
      outfile.precision(6);
      outfile << " " << v_ba_bg(0, 0) << " " << v_ba_bg(1, 0) << " " << v_ba_bg(2, 0) << " " << v_ba_bg(3, 0) << " " << v_ba_bg(4, 0) << " "
              << v_ba_bg(5, 0) << " " << v_ba_bg(6, 0) << " " << v_ba_bg(7, 0) << " " << v_ba_bg(8, 0) << std::endl;
    } else {
      outfile << std::endl;
    }
  }
  ...
  Eigen::Matrix<double, 9, 1> v_ba_bg;
};
