...
void Loader::load_data(std::string path_traj, std::vector<double> &times, std::vector<Eigen::Matrix<double, 7, 1>> &poses,
                       std::vector<Eigen::Vector3d> &vs, std::vector<Eigen::Vector3d> &bas,std::vector<Eigen::Vector3d> &bgs) {
  ...
  while (std::getline(file, current_line)) {
    ...
    if (i >= 17) {
      ...
      poses.push_back(data.block(1, 0, 7, 1));
      // covariance values
      Eigen::Vector3d v, ba, bg;
      v << data(8), data(9), data(10);
      ba << data(11), data(12), data(13);
      bg << data(14), data(15), data(16);
      vs.push_back(v);
      bas.push_back(ba);
      bgs.push_back(bg);
    } else if (i >= 8) {
  ...
  // Assert that they are all equal
  if (!vs.empty() && (times.size() != vs.size() || times.size() != bas.size() || times.size() != bgs.size())) {
    PRINT_ERROR(RED "[LOAD]: Parsing error, timestamps covariance size do not match!!\n" RESET);
    PRINT_ERROR(RED "[LOAD]: %s\n" RESET, path_traj.c_str());
    std::exit(EXIT_FAILURE);
  }
...
void Loader::pose_seg_by_length(const std::vector<double> &times_gt,const std::vector<double> &times_esi,
          const std::vector<Eigen::Matrix<double, 7, 1>> &poses_gt,
          const std::vector<Eigen::Matrix<double, 7, 1>> &poses_esi, double length,
          std::vector<std::vector<Eigen::Matrix<double, 7, 1>>> &gtposes_seg,
          std::vector<std::vector<Eigen::Matrix<double, 7, 1>>> &esiposes_seg,
          std::vector<std::vector<double>> &gttimes_seg, std::vector<std::vector<double>> &esitimes_seg) {

  // Check
  assert(poses_gt.size() == poses_esi.size());
  assert(times_gt.size() == times_esi.size());
  assert(times_gt.size() == poses_esi.size());
  // Loop through every pose and append its segment
  double distance = 0.0;
  int seg_num = 0;
  auto it_gt = poses_gt.begin();
  auto it_esi = poses_esi.begin();
  auto it_gtt = times_gt.begin();
  auto it_esit = times_esi.begin();
  Eigen::Matrix<double, 7, 1> last_pose;
  std::vector<Eigen::Matrix<double, 7, 1>> gt,esi;
  std::vector<double> gtt,esit;
  gtposes_seg.push_back(gt);
  esiposes_seg.push_back(esi);
  gttimes_seg.push_back(gtt);
  esitimes_seg.push_back(gtt);
  while(it_gt != poses_gt.end()){
    gtposes_seg.at(seg_num).push_back(*it_gt);
    esiposes_seg.at(seg_num).push_back(*it_esi);
    gttimes_seg.at(seg_num).push_back(*it_gtt);
    esitimes_seg.at(seg_num).push_back(*it_esit);
    last_pose = *it_gt;
    it_gt++;
    it_esi++;
    if(it_gt != poses_gt.end() && distance < length)distance += ((*it_gt).block(0, 0, 3, 1) - last_pose.block(0, 0, 3, 1)).norm();
    else if(it_gt != poses_gt.end() && distance >= length){
      seg_num++;
      distance = 0.0;
      std::vector<Eigen::Matrix<double, 7, 1>> gt,esi;
      std::vector<double> gtt,esit;
      ...
      gtposes_seg.pop_back();
      esiposes_seg.pop_back();
      gttimes_seg.pop_back();
      esitimes_seg.pop_back();
      break;
    }
    else break;
  }
