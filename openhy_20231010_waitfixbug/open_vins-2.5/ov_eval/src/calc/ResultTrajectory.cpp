...
void ResultTrajectory::calculate_ate_align(Statistics &error_ori, Statistics &error_pos) {
...
void ResultTrajectory::calculate_ate_alignbegin(const std::vector<double> &segment_lengths,
                                     std::map<double, std::pair<Statistics, Statistics>> &error_ate) {

  /* visual the part align
  #ifdef HAVE_PYTHONLIBS
    // Plot this figure
  ...
  */
  ...
  for (const double &distance : segment_lengths) {
    ...
    for (size_t id_start = 0; id_start < comparisons.size(); id_start++) {
      ...
      double s_ESTtoGT;
      std::vector<Eigen::Matrix<double, 7, 1>> est_align_part, gt_align_part,est_align_to_gt_part;
      est_align_part.insert(est_align_part.end(),est_poses_aignedtoGT.begin() + id_start, est_poses_aignedtoGT.begin() + id_end);
      gt_align_part.insert(gt_align_part.end(),gt_poses.begin() + id_start, gt_poses.begin() + id_end);
      AlignTrajectory::align_trajectory(est_align_part, gt_align_part, R_ESTtoGT, t_ESTinGT, s_ESTtoGT, "se3begin");
      ...
      PRINT_DEBUG("[TRAJ]: q_ESTtoGT = %.3f, %.3f, %.3f, %.3f | p_ESTinGT = %.3f, %.3f, %.3f | s = %.2f\n", q_ESTtoGT(0), q_ESTtoGT(1),
                  q_ESTtoGT(2), q_ESTtoGT(3), t_ESTinGT(0), t_ESTinGT(1), t_ESTinGT(2), s_ESTtoGT);
      // Finally lets calculate the aligned trajectories
      for (size_t i = 0; i < est_align_part.size(); i++) {
        Eigen::Matrix<double, 7, 1> pose_ESTinGT;
        pose_ESTinGT.block(0, 0, 3, 1) = s_ESTtoGT * R_ESTtoGT * est_align_part.at(i).block(0, 0, 3, 1) + t_ESTinGT;
        pose_ESTinGT.block(3, 0, 4, 1) = ov_core::quat_multiply(est_align_part.at(i).block(3, 0, 4, 1), ov_core::Inv(q_ESTtoGT));
        est_align_to_gt_part.push_back(pose_ESTinGT);
      }
      /*
      // visual the part align
      if(distance > 601 && distance < 799){
        #ifdef HAVE_PYTHONLIBS
          // Plot the position trajectories
          plot_xy_positions("gt", "black", gt_align_part);
          plot_xy_positions("est_align", "blue", est_align_to_gt_part);
          plot_xy_positions("est_ori", "green", est_align_part);

          // Display to the user
          matplotlibcpp::legend();
          matplotlibcpp::show();
          matplotlibcpp::pause(1e-6);
          matplotlibcpp::clf();
      ...
      */
      //==============================================================================
      // Calculate orientation error
      Eigen::Matrix3d e_R = ov_core::quat_2_Rot(est_align_to_gt_part.back().block(3, 0, 4, 1)).transpose() *
                            ov_core::quat_2_Rot(gt_align_part.back().block(3, 0, 4, 1));
      double ori_err = 180.0 / M_PI * ov_core::log_so3(e_R)(2);

      // Calculate position error
      double pos_err = (gt_align_part.back().block(0, 0, 2, 1) - est_align_to_gt_part.back().block(0, 0, 2, 1)).norm();
    ...
    error_ori.calculate();
    error_pos.calculate();
    error_ate.insert({distance, {error_ori, error_pos}});
  }
}
...
void ResultTrajectory::calculate_rpe(const std::vector<double> &segment_lengths,
  ...
  for (const double &distance : segment_lengths) {
    ...
    for (size_t id_start = 0; id_start < comparisons.size(); id_start++) {
      ...
      error_pos.timestamps.push_back(est_times.at(id_start));
      double error_T,error_t;
      Eigen::Vector2d error_tt;
      error_T = T_error_in_w.block(0, 3, 2, 1).norm();
      error_tt = ((T_c2-T_c1)-(T_m2-T_m1)).block(0,3,2,1);
      error_t = error_tt.norm();
      if(error_t >= error_T)error_pos.values.push_back(error_T);
      else error_pos.values.push_back(error_t);
...
void ResultTrajectory::calculate_rpe_ep(const std::vector<double> &segment_lengths,
                                     std::map<double, std::pair<Statistics, Statistics>> &error_rpe) {
...
void ResultTrajectory::calculate_rpe_align(const std::vector<double> &segment_lengths,
                                     std::map<double, std::pair<Statistics, Statistics>> &error_rpe) {
  ...
  */
  ...
  for (const double &distance : segment_lengths) {
    ...
    std::vector<int> comparisons = compute_comparison_indices_length(accum_distances, distance, max_dist_diff);
    assert(comparisons.size() == gt_poses.size());
    std::vector<Eigen::Matrix<double, 7, 1>> est_poses_part_align_to_GT,est_poses_part_align_to_GT_part;
    std::vector<Eigen::Matrix<double, 7, 1>> GT,GT_part;
    // Loop through each relative comparison
    for (size_t id_start = 0; id_start < comparisons.size(); id_start++) {
      ...
      est_align_part.insert(est_align_part.end(),est_poses_aignedtoGT.begin() + id_start, est_poses_aignedtoGT.begin() + id_end);
      gt_align_part.insert(gt_align_part.end(),gt_poses.begin() + id_start, gt_poses.begin() + id_end);
      AlignTrajectory::align_trajectory(est_align_part, gt_align_part, R_ESTtoGT, t_ESTinGT, s_ESTtoGT, "se3");
      ...
      */
      /* if get the first one
      est_poses_part_align_to_GT.push_back(est_align_to_gt_part.front());
      est_poses_part_align_to_GT_part = est_align_to_gt_part;
      GT.push_back(gt_align_part.front());
      GT_part = gt_align_part;
      */
      /*else get the mid one*/
      if(est_poses_part_align_to_GT.empty()){
        est_poses_part_align_to_GT.insert(est_poses_part_align_to_GT.end(),est_align_to_gt_part.begin(),est_align_to_gt_part.begin()+(est_align_to_gt_part.size()/2));
        GT.insert(GT.end(),gt_align_part.begin(),gt_align_part.begin()+(gt_align_part.size() / 2));
      }
      est_poses_part_align_to_GT.push_back(*(est_align_to_gt_part.begin()+(est_align_to_gt_part.size()/2)));
      est_poses_part_align_to_GT_part = est_align_to_gt_part;
      GT.push_back(*(gt_align_part.begin()+(gt_align_part.size() / 2)));
      GT_part = gt_align_part;
      //===============================================================================
      // Get T I1 to world EST at beginning of subtrajectory (at state idx)
      Eigen::Matrix4d T_c1 = Eigen::Matrix4d::Identity();
      T_c1.block(0, 0, 3, 3) = ov_core::quat_2_Rot(est_align_to_gt_part.front().block(3, 0, 4, 1)).transpose();
      T_c1.block(0, 3, 3, 1) = est_align_to_gt_part.front().block(0, 0, 3, 1);

      // Get T I2 to world EST at end of subtrajectory starting (at state comparisons[idx])
      Eigen::Matrix4d T_c2 = Eigen::Matrix4d::Identity();
      T_c2.block(0, 0, 3, 3) = ov_core::quat_2_Rot(est_align_to_gt_part.back().block(3, 0, 4, 1)).transpose();
      T_c2.block(0, 3, 3, 1) = est_align_to_gt_part.back().block(0, 0, 3, 1);
      ...
      // Get T I1 to world GT at beginning of subtrajectory (at state idx)
      Eigen::Matrix4d T_m1 = Eigen::Matrix4d::Identity();
      T_m1.block(0, 0, 3, 3) = ov_core::quat_2_Rot(gt_align_part.front().block(3, 0, 4, 1)).transpose();
      T_m1.block(0, 3, 3, 1) = gt_align_part.front().block(0, 0, 3, 1);

      // Get T I2 to world GT at end of subtrajectory starting (at state comparisons[idx])
      Eigen::Matrix4d T_m2 = Eigen::Matrix4d::Identity();
      T_m2.block(0, 0, 3, 3) = ov_core::quat_2_Rot(gt_align_part.back().block(3, 0, 4, 1)).transpose();
      T_m2.block(0, 3, 3, 1) = gt_align_part.back().block(0, 0, 3, 1);
    ...
    est_poses_part_align_to_GT.insert(est_poses_part_align_to_GT.end(),
        est_poses_part_align_to_GT_part.begin() + (est_poses_part_align_to_GT_part.size()/2) + 1,est_poses_part_align_to_GT_part.end());
    est_poses_part_align_to_GT_map[distance] = est_poses_part_align_to_GT;
    GT.insert(GT.end(),GT_part.begin() + (GT_part.size()/2) + 1,GT_part.end());
    GT_map[distance] = GT;
    // Update stat information
    error_ori.calculate();
    error_pos.calculate();
