...
public:
  ...
  ResultTrajectory(std::vector<double> esttimes,std::vector<double> gttimes,
  std::vector<Eigen::Matrix<double, 7, 1>> estposes, std::vector<Eigen::Matrix<double, 7, 1>> gtposes,
  std::vector<Eigen::Matrix<double, 7, 1>> estposes_aignedtoGT,std::vector<Eigen::Matrix<double, 7, 1>> gtposes_aignedtoEST)
  :est_times(esttimes),gt_times(gttimes),est_poses(estposes),gt_poses(gtposes),
  est_poses_aignedtoGT(estposes_aignedtoGT),gt_poses_aignedtoEST(gtposes_aignedtoEST){}
  /**
  ...
  /**
   * @brief Computes the Absolute Trajectory Error (ATE) for align trajectory in the 2d x-y plane.
   *
   * This will first do our alignment of the two trajectories.
   * We just grab the yaw component of the orientation and the xy plane error.
   * Then at each point the error will be calculated and normed as follows:
   ...
   * @param error_pos Error values for the position (xy error)
   */
  void calculate_ate_align(Statistics &error_ori, Statistics &error_pos);

  /**
   * @brief Computes the ate for this trajectory endpoint
   *
   ...
   * @param error_ate Map of segment lengths => errors for that length (orientation and position)
   */
  void calculate_ate_alignbegin(const std::vector<double> &segment_lengths,std::map<double, std::pair<Statistics, Statistics>> &error_ate);
  ...
  /**
   * @brief Computes the Relative Pose Error (RPE) only for this trajectory endpoints
   *
   * For the given set of segment lengths, this will split the trajectory into segments.
   * From there it will compute the relative pose error for all segments.
   * These are then returned as a map for each segment length.
   ...
   * @param error_rpe Map of segment lengths => errors for that length (orientation and position)
   */
  void calculate_rpe_ep(const std::vector<double> &segment_lengths, std::map<double, std::pair<Statistics, Statistics>> &error_rpe);

  /**
   * @brief Computes the Relative Pose Error (RPE)  for slice align trajectory 
   *
   * For the given set of segment lengths, this will split the trajectory into segments.
   * From there it will compute the relative pose error for all segments.
   * These are then returned as a map for each segment length.
   ...
   * @param error_rpe Map of segment lengths => errors for that length (orientation and position)
   */
  void calculate_rpe_align(const std::vector<double> &segment_lengths, std::map<double, std::pair<Statistics, Statistics>> &error_rpe);
  ...
  void get_data(std::vector<double> &esttimes,std::vector<double> &gttimes,
  std::vector<Eigen::Matrix<double, 7, 1>> &estposes, std::vector<Eigen::Matrix<double, 7, 1>> &gtposes,
  std::vector<Eigen::Matrix<double, 7, 1>> &estposes_aignedtoGT,std::vector<Eigen::Matrix<double, 7, 1>> &gtposes_aignedtoEST){
    esttimes = est_times;
    gttimes = gt_times;
    estposes = est_poses;
    gtposes = gt_poses;
    estposes_aignedtoGT = est_poses_aignedtoGT;
    gtposes_aignedtoEST = gt_poses_aignedtoEST;
  }
  
  // Part align traj
  std::map<double,std::vector<Eigen::Matrix<double, 7, 1>>> est_poses_part_align_to_GT_map;
  std::map<double,std::vector<Eigen::Matrix<double, 7, 1>>> GT_map;
