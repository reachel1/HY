...
void AlignTrajectory::align_trajectory(const std::vector<Eigen::Matrix<double, 7, 1>> &traj_es,
  ...
  } else if (method == "se3begin"){
    s = 1;
    align_se3_begin(traj_es, traj_gt, R, t);
  }else if (method == "sim3") {
...
void AlignTrajectory::align_se3_begin(const std::vector<Eigen::Matrix<double, 7, 1>> &traj_es,
                                      const std::vector<Eigen::Matrix<double, 7, 1>> &traj_gt, Eigen::Matrix3d &R, Eigen::Vector3d &t,
                                      int n_aligned) {
  ...
  if (n_aligned == 1) {
    ...
    assert(!traj_es.empty());
    std::vector<Eigen::Vector3d> pos_est, pos_gt;
    for (size_t i = 0; i < traj_es.size() && i < traj_gt.size() && distance <= 3; i++) {

      pos_est.push_back(traj_es.at(i).block(0, 0, 3, 1));
      pos_gt.push_back(traj_gt.at(i).block(0, 0, 3, 1));
      distance += (traj_gt.at(i+1).block(0, 0, 2, 1)-traj_gt.at(i).block(0, 0, 2, 1)).norm();
    }
