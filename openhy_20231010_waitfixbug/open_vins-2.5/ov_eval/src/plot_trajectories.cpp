...
void plot_xyz_positions(const std::vector<std::string> &names, const std::vector<std::string> &colors, const std::vector<std::vector<double>> &times,
                      const std::vector<std::vector<Eigen::Matrix<double, 7, 1>>> &poses) {
  // Finally plot
  ...
  for(int i = 0;i < times.size();i++){ 
    ...
    for (int j = 0; j < poses.at(i).size(); j++) {
      time.push_back(times.at(i).at(j));
      x.push_back(poses.at(i).at(j)(0));
    }
    // Paramters for our line
    std::map<std::string, std::string> params;
    params.insert({"label", names.at(i)});
    params.insert({"linestyle", "-"});
    params.insert({"color", colors.at(i)});
    matplotlibcpp::plot(time, x, params);
  }
  matplotlibcpp::subplot(3,1,2);
  matplotlibcpp::title("y");
  for(int i = 0;i < times.size();i++){
    std::vector<double> time, x;
    for (int j = 0; j < poses.at(i).size(); j++) {
      time.push_back(times.at(i).at(j));
      x.push_back(poses.at(i).at(j)(1));
    }
    // Paramters for our line
    std::map<std::string, std::string> params;
    params.insert({"label", names.at(i)});
  ...
  matplotlibcpp::title("z");
  for(int i = 0;i < times.size();i++){
    std::vector<double> time, x;
    for (int j = 0; j < poses.at(i).size(); j++) {
      time.push_back(times.at(i).at(j));
      x.push_back(poses.at(i).at(j)(2));
    }
    // Paramters for our line
    std::map<std::string, std::string> params;
    params.insert({"label", names.at(i)});
...
void plot_eular(const std::vector<std::string> &names, const std::vector<std::string> &colors, const std::vector<std::vector<double>> &times,
                      const std::vector<std::vector<Eigen::Matrix<double, 7, 1>>> &poses){
  ...
  for(int i = 0;i < times.size();i++){
    ...
    for (int j = 0; j < poses.at(i).size(); j++) {
      time.push_back(times.at(i).at(j));
      x.push_back(ov_core::rot2rpy(ov_core::quat_2_Rot(poses.at(i).at(j).block(3,0,4,1)))(0)/M_PI*180);
    }
    // Paramters for our line
    std::map<std::string, std::string> params;
    params.insert({"label", names.at(i)});
  ...
  for(int i = 0;i < times.size();i++){
    ...
    for (int j = 0; j < poses.at(i).size(); j++) {
      time.push_back(times.at(i).at(j));
      x.push_back(ov_core::rot2rpy(ov_core::quat_2_Rot(poses.at(i).at(j).block(3,0,4,1)))(1)/M_PI*180);
    }
    // Paramters for our line
    std::map<std::string, std::string> params;
    params.insert({"label", names.at(i)});
  ...
  for(int i = 0;i < times.size();i++){
    ...
    for (int j = 0; j < poses.at(i).size(); j++) {
      time.push_back(times.at(i).at(j));
      x.push_back(ov_core::rot2rpy(ov_core::quat_2_Rot(poses.at(i).at(j).block(3,0,4,1)))(2)/M_PI*180);
    }
    // Paramters for our line
    std::map<std::string, std::string> params;
    params.insert({"label", names.at(i)});
...
// Will plot the z 3d position of the pose trajectories
void plot_v_ba_bg(const std::vector<std::string> &names, const std::vector<std::string> &colors, const std::vector<std::vector<double>> &times,
                      const std::vector<std::vector<Eigen::Vector3d>> xs) {
  // sub plot
  matplotlibcpp::subplot(3,1,1);
  matplotlibcpp::title("x_axis");
  for(int i = 0;i < times.size();i++){
    std::vector<double> time, x;
    for (int j = 0; j < times.at(i).size(); j++) {
      time.push_back(times.at(i).at(j));
      x.push_back(xs.at(i).at(j)(0));
    }
    // Paramters for our line
    std::map<std::string, std::string> params;
    params.insert({"label", names.at(i)});
  ...
  matplotlibcpp::subplot(3,1,2);
  matplotlibcpp::title("y_axis");
  for(int i = 0;i < times.size();i++){
    std::vector<double> time, x;
    for (int j = 0; j < times.at(i).size(); j++) {
      time.push_back(times.at(i).at(j));
      x.push_back(xs.at(i).at(j)(1));
    }
    // Paramters for our line
    std::map<std::string, std::string> params;
    params.insert({"label", names.at(i)});
  ...
  matplotlibcpp::subplot(3,1,3);
  matplotlibcpp::title("z_axis");
  for(int i = 0;i < times.size();i++){
    std::vector<double> time, x;
    for (int j = 0; j < times.at(i).size(); j++) {
      time.push_back(times.at(i).at(j));
      x.push_back(xs.at(i).at(j)(2));
    }
    // Paramters for our line
    std::map<std::string, std::string> params;
    params.insert({"label", names.at(i)});
...
int main(int argc, char **argv) {
  ...
  std::vector<std::vector<Eigen::Vector3d>> vs, bas, bgs;
  for (int i = 2; i < argc; i++) {

    // Read in trajectory data
    std::vector<double> times_temp;
    std::vector<Eigen::Matrix<double, 7, 1>> poses_temp;
    std::vector<Eigen::Vector3d> vs_temp, bas_temp, bgs_temp;
    ov_eval::Loader::load_data(argv[i], times_temp, poses_temp, vs_temp, bas_temp, bgs_temp);
    ...
    vs.push_back(vs_temp);
    bas.push_back(bas_temp);
    bgs.push_back(bgs_temp);
  }
  ...
  // Plot the position trajectories
  plot_xyz_positions(names, colors, times, poses);

  // Display to the user
  matplotlibcpp::xlabel("timestamp (sec)");
  matplotlibcpp::ylabel("xyz-axis (m)");
  matplotlibcpp::legend();
  matplotlibcpp::show(false);
  ...
  // Plot the position trajectories
  plot_eular(names, colors, times, poses);
  ...
  // Plot the position trajectories
  plot_v_ba_bg(names, colors, times, vs);

  // Display to the user
  matplotlibcpp::xlabel("timestamp (sec)");
  matplotlibcpp::ylabel("v(m/s)");
  matplotlibcpp::legend();
  matplotlibcpp::show(false);
  ...
  // Plot the position trajectories
  plot_v_ba_bg(names, colors, times, bas);

  // Display to the user
  matplotlibcpp::xlabel("timestamp (sec)");
  matplotlibcpp::ylabel("bias_a");
  matplotlibcpp::legend();
  matplotlibcpp::show(false);
  ...
  // Plot the position trajectories
  plot_v_ba_bg(names, colors, times, bgs);

  // Display to the user
  matplotlibcpp::xlabel("timestamp (sec)");
  matplotlibcpp::ylabel("bias_g");
  matplotlibcpp::legend();
